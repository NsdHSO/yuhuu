        ...
        { text: t("common.cancel"), style: "cancel" },
        {
          ...
          onPress: () => {
            deleteMutation.mutate(id, {
              onSuccess: () =>
                Alert.alert(t("common.success"), t("milestones.deleteSuccess")),
              onError: (e) =>
  ...
  const handleSubmit = () => {
    if (mode === "create") {
      ...
      const data: CreateSpiritualMilestoneInput = {
        ...
        ...(formData.milestone_date
          ? { milestone_date: formData.milestone_date }
          : {}),
        ...(formData.location.trim()
          ? { location: formData.location.trim() }
          : {}),
        ...(formData.officiant.trim()
          ? { officiant: formData.officiant.trim() }
          : {}),
      ...
      createMutation.mutate(data, {
        ...
        onError: (e) => {
          if (isConflictError(e)) {
            ...
            Alert.alert(
              t("common.error"),
              getErrorMessage(e, t("milestones.createError")),
            );
          }
        },
      });
      ...
      updateMutation.mutate(
        { id: editingId, data },
        {
          ...
          onError: (e) => {
            Alert.alert(
              t("common.error"),
              getErrorMessage(e, t("milestones.updateError")),
            );
          },
        },
      );
  ...
  const renderForm = () => (
    <View testID="milestone-form" style={{ gap: 12, marginTop: 8 }}>
          ...
          >
            {MILESTONE_TYPES.map((type) => (
              <Pressable
                key={type}
                onPress={() =>
                  setFormData((prev) => ({ ...prev, milestone_type: type }))
                }
                style={({ pressed }) => ({
                  ...
                  shadowColor:
                    formData.milestone_type === type ? activeColor : "#000",
              ...
              >
                <ThemedText
                  style={{
                    fontSize: 15,
                    fontWeight:
                      formData.milestone_type === type ? "700" : "600",
                    letterSpacing: 0.3,
                    color:
                      formData.milestone_type === type
                        ? scheme === "dark"
      ...
      {mode === "edit" && (
        ...
        >
          {getMilestoneIcon(formData.milestone_type)}{" "}
          {getMilestoneLabel(formData.milestone_type)}
        </ThemedText>
      )}
      ...
      <GlassInput
        ...
        onChangeText={(text) =>
          setFormData((prev) => ({ ...prev, officiant: text }))
        }
  ...
  return (
    ...
    >
      {isLoading ? (
          ...
          {milestones.map((milestone) =>
            mode === "edit" && editingId === milestone.id ? (
              ...
              >
                ...
                {milestone.notes && (
                  <ThemedText
                    style={{ fontSize: 13, marginTop: 4, fontStyle: "italic" }}
                  >
                    {milestone.notes}
                  </ThemedText>
                )}
        ...
        <ThemedText
          style={{ textAlign: "center", color: Colors[scheme].tabIconDefault }}
        >
