...
describe('Android Workflow - Full Step Ordering (E2E)', () => {
    ...
    it('should have steps in the correct overall order', () => {
        const stepOrder = [
            ...
            { name: 'node setup', finder: (s: WorkflowStep) => s.uses?.includes('setup-node') || s.name?.toLowerCase().includes('node') },
            ...
            { name: 'env validation', finder: (s: WorkflowStep) => s.name?.toLowerCase().includes('validate environment') },
            { name: 'prebuild cache', finder: (s: WorkflowStep) => s.uses?.includes('actions/cache') && String(s.with?.path || '').includes('android/') },
